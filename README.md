...
**한 길 물낯**은 로컬 초상 이미지 perturbation 실험과 방어 평가를 위한 Android 앱입니다. 선택한 이미지에 보호용 변형을 적용하고, 복원 시도 이후에도 얼굴 감지 방해 효과가 유지되는지 기기 안에서 점검합니다.
...
- Android 갤러리 또는 공유 시트에서 초상 이미지를 불러옵니다.
...
- `PASS`: 기본 보호 이미지가 설정된 로컬 probe를 방해했습니다.
- `HELD`: 복원 시도 이후에도 방해 효과가 유지됐습니다.
- `BROKEN`: 복원 시도 후 충분한 신호가 회복되어 더 강한 조정이나 다른 방법이 필요합니다.
...
| 처리 시간 개선 | 약 44.4% 감소 |
| 188장 실험 중 대응 보호본 없음 | 30장 |

대응 보호본이 있고 원본에서 얼굴 감지가 성립한 사례를 기준으로 한 방어 성공률은 다음과 같습니다.

| 검출기 | 성공 | 성공률 |
| --- | ---: | ---: |
| OpenCV Haar | 50 / 111 | 45.0% |
| MTCNN | 142 / 150 | 94.7% |
| InsightFace | 122 / 158 | 77.2% |
